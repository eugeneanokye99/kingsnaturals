...
class User
{
    ...
    public function getUserDetails($mysqli, $userId)
    {
        ...
        if ($result->num_rows > 0) {
            $userData = $result->fetch_assoc(); 
            echo json_encode(['status' => 'success', 'data' => $userData], JSON_PRETTY_PRINT);
        } else {
    ...
    public function getProducts($mysqli) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo json_encode(['status' => 'success', 'data' => $products], JSON_PRETTY_PRINT);
        } else {
            // No products found
